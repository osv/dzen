...
`dzen2` - scriptable X11 rendering backend for status bars, notifications and menus
...
Appearance:
    ...
    -fn-preload FONT1,FONT2,...  preload fonts for numbered aliases
...
`-fn-preload FONT1,FONT2,...` loads up to 64 comma-separated fonts before
processing input. Aliases are numbered from zero: `^fn(dfnt0)` and `^fn(0)`
select the first font, `^fn(dfnt1)` and `^fn(1)` the second. Spaces around font
names are trimmed. Use x: for X11 core fonts or xft: for XFT fonts; the latter
requires an XFT-enabled build. Preloading works with either backend and does
not replace the normal font selected by `-fn`. Use `^fn()` to restore it.
...
### Colors:
    ...
    ^fn(dfnt0)         Select the first font from -fn-preload
    ^fn(0)             Numeric alias for the same preloaded font
...
### Supported actions:
    ...
    menuprint           write selected menu entry to STDOUT after parsing formatting
    menuprint_noparse   write selected menu entry to STDOUT with formatting commands intact
...
Preload fonts and switch between them within a multilingual greeting:

    printf '%s\n' '^fn(0)Hello, World! ^fn(1)今日は！ ^fn(2)😊🚀 ^fn(3)12345' |
        dzen2 -fn 'xft:DejaVu Sans' \
            -fn-preload "xft:DejaVu Sans,\
                xft:Noto Sans CJK JP,\
                xft:Noto Color Emoji,\
                x:fixed" \
            -h 32 -w 600 -ta l -p 3

This example requires an XFT-enabled build and the listed fonts installed.
Preloaded fonts can be selected by name as well as by numbered alias.
The fourth font uses x: to select an X11 core font;
`^fn(3)` renders 12345 with it.
