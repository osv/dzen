    ...
    if (dbar->mode == textual) {
        ...
        case outlined:
            if (dbar->segb == 0) {
                fprintf(stream, "%s^ib(1)^fg(%s)^ro(%dx%d)^p(%d)^fg(%s)^r(%dx%d)^p(%d)^ib(0)^fg()%s",
                        dbar->label ? dbar->label : "", dbar->bg, (int)dbar->width, dbar->height,
                        -1 * (dbar->width - 2), dbar->fg, (int)l > dbar->width - 4 ? dbar->width - 4 : (int)l,
                        dbar->height - 4 > 0 ? dbar->height - 4 : 1, dbar->width - (int)l - 1, dbar->pnl ? "\n" : "");
            } else {
                segs  = dbar->width / (dbar->segw + dbar->segb);
                segsa = rp * segs / 100;

                fprintf(stream, "%s^ib(1)^fg(%s)^ro(%dx%d)^p(%d)", dbar->label ? dbar->label : "", dbar->bg,
                        (int)dbar->width, dbar->height, -1 * (dbar->width - 2));
                for (i = 0; i < segs; i++) {
                    if (i < segsa)
                        fprintf(stream, "^fg(%s)^r(%dx%d+%d+%d')", dbar->fg, dbar->segw,
                                dbar->height - 4 > 0 ? dbar->height - 4 : 1, i ? dbar->segb : 0, 0);
                    else
                        break;
                }
        ...
        case graph:
            dbar->gc           = dbar->gc < MAX_GRAPH_VALS &&
                               (dbar->gs == 0 ? dbar->gc : dbar->gc * dbar->gs + dbar->gc * dbar->gw) < dbar->width
                                     ? ++dbar->gc
                                     : 0;
            dbar->gb[dbar->gc] = l;
            ...
            for (i = dbar->gc + 1; i < MAX_GRAPH_VALS && (i * (dbar->gs + dbar->gw)) < dbar->width; ++i) {
                ...
                fprintf(stream, "^fg(%s)^p(%d)^r(%dx%d+0-%d)", dbar->fg, dbar->gs, dbar->gw, (int)dbar->gb[i],
                        (int)dbar->gb[i] + 1);
            }
        ...
        case pie:
            fprintf(stream, "^ib(1)^fg(%s)^c(%d)^p(-%d)^fg(%s)^c(%d-%d)%s", dbar->bg, dbar->width, dbar->width,
                    dbar->fg, dbar->width, (int)(rp * 360 / 100), dbar->pnl ? "\n" : "");
            break;
        ...
        default:
            if (dbar->segb == 0)
                ...
                for (i = 0; i < segs; i++) {
                    if (i < segsa)
                        fprintf(stream, "^fg(%s)^r(%dx%d+%d+%d)", dbar->fg, dbar->segw, dbar->height,
                                i ? dbar->segb : 0, 0);
                    else
                        fprintf(stream, "^fg(%s)^r(%dx%d+%d+%d)", dbar->bg, dbar->segw, dbar->height,
                                i ? dbar->segb : 0, 0);
                }
