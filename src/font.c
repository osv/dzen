...
static Fnt *font_load(const char *fontstr) {
    if (!fontstr || !*fontstr) return NULL;
...
void font_draw_text_xft(Drawable drawable, int x, int y, const char *text, unsigned int len, const char *color, int screen) {
    Fnt *font = font_get_current();
    if (!font || !font->xftfont) return;
    
    /* Create or update XftDraw */
    if (!g_xftdraw) {
        g_xftdraw = XftDrawCreate(g_display, drawable, g_visual, g_colormap);
    } else {
        XftDrawChange(g_xftdraw, drawable);
    }
    
    /* Allocate color */
    XftColor xftcolor;
    if (!XftColorAllocName(g_display, g_visual, g_colormap, color, &xftcolor)) {
        return;
    }
    
    /* Draw text */
    XftDrawStringUtf8(g_xftdraw, &xftcolor, font->xftfont, x, y + font->ascent, (const FcChar8 *)text, len);
    
    /* Free color */
    XftColorFree(g_display, g_visual, g_colormap, &xftcolor);
}
#endif
