...
printer() {
    local max_frames=${1:-0}  # Default to infinite (0 means unlimited)
    i=0
    while [ "$max_frames" -eq 0 ] || [ $i -lt "$max_frames" ]; do
        i=$((i + 1))
        printf "%s %s\n" \
...
test_bench() {
    ...
    printer 10000 | ./src/dzen2 -dock -ta l -h 30 \
        -y 40 \
        -fg "#33e" \
        -bg "#e7e7e7" \
        -e "button3=exit"
    ...
    local fps=$(echo "scale=2; 10000 / $duration" | bc -l)
